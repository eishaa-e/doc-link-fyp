    ...
    try {
        ...
        if (query === "upcoming") {
            appointments = appointments.filter(
                (appointment) => (appointment.status !== "CANCELLED" && new Date(appointment.date) > currentDate),
            );
        } else if (query === "past") {
            appointments = appointments.filter(
                (appointment) => (appointment.status !== "CANCELLED" && new Date(appointment.date) <= currentDate),
...
exports.updateAppointmentStatus = async (req, res) => {
    ...
    try {
        ...
        if (req.user.role === "doctor" && appointment.doctor_id !== doctor._id) {
        ...
        if (req.user.role === "patient" && appointment.patient_id !== patient._id) {
            return res.status(403).json({
                message: "You are not authorized to update this appointment status",
            });
        }
        ...
        if (status === "BOOKED" && appointment.patient_id === patient._id) {
            return res.status(403).json({
                message: "You are not authorized to update this appointment status",
            });
        }
