...
def predict_kidney_stone():
    ...
    # Perform prediction
    results = kidney_stone_model.predict(source=image_path, conf=0.25)
...
def predict_brain_tumor():
    ...
    # Perform prediction
    results = brain_tumor_model.predict(source=image_path, conf=0.25)
    ...
    for box in predictions:
        x1, y1, x2, y2, conf, cls = box
        x1, y1, x2, y2 = int(x1), int(y1), int(x2), int(y2)
        label = "Tumor" if int(cls) == 0 else "No Tumor"

        # Draw bounding box on the image
        color = (0, 255, 0) if label == "Tumor" else (0, 0, 255)
        cv2.rectangle(image, (x1, y1), (x2, y2), color, 2)
        cv2.putText(image, f"{label} {conf:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
